...
func main() {
	//dsn := "host=db user=postgres password=1234 dbname=SurveyDB port=5432 sslmode=prefer TimeZone=Asia/Shanghai"
	db, err := gorm.Open(postgres.Open("postgresql://postgres:1234@db:5432/surveyDB"), &gorm.Config{Logger: initLogger()})
	if err != nil {
